...
@section('title', __("admin.mood_{$mood}") . ' — ' . $text['mod_label'] . ' — ' . $siteName)
...
<section class="relative flex min-h-dvh flex-col items-center justify-center px-6 py-16 text-center
    ...
    <h1 class="mt-4 font-serif text-4xl uppercase tracking-[0.15em] sm:text-5xl">{{ __("admin.mood_{$mood}") }}</h1>
    ...
    <div class="mt-12 grid w-full max-w-md grid-cols-1 gap-3"
         x-data="surpriseBox(@js($mood), @js([
             'photo' => __('admin.type_photo'),
             'note' => __('admin.type_note'),
             'quote' => __('admin.type_quote'),
             'activity' => __('admin.type_activity'),
             'music' => __('admin.type_music'),
             'memory' => __('admin.type_memory'),
         ]))">
        @if($mood === 'good')
        ...
        <div class="mt-2" x-cloak x-show="item" x-transition.opacity.duration.500ms>
            <template x-if="item">
                <div class="card p-6 text-left">
                    <p class="text-[10px] uppercase tracking-widest text-night-500" x-text="typeLabels[item.type] ?? item.type"></p>
                    ...
                    <template x-if="item.embed"><iframe :src="item.embed" class="mt-3 h-[352px] w-full rounded-lg" frameborder="0" allowfullscreen title="{{ __('common.surprise_music') }}"></iframe></template>
